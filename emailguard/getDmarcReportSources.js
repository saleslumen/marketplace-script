/**
 * @description Get DMARC report sources for a domain between two dates.
 * @param {Object} input
 * @returns {Object}
 */
async function getDmarcReportSources(input) {
  ...
  let path = "/api/v1/dmarc-reports/domains/{domain_uuid}/dmarc-sources";
  path = path.replace("{domain_uuid}", encodeURIComponent(domain_uuid));
  const body = {};
  body.start_date = start_date;
  body.end_date = end_date;
  return runAuthed(path, "GET", body, "DMARC_SOURCES", "DMARC_SOURCES_FAILED");
}
