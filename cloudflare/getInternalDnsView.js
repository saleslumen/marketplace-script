/**
 * @description DNS internal view details. GET /accounts/{account_id}/dns_settings/views/{view_id}.
 * @param {Object} input
 ...
 */
async function getInternalDnsView(input) {
  const req = inputObject(input);
  const accountId = requiredString(req, "account_id");
  const viewId = requiredString(req, "view_id");
  return cloudflareRequest(`/accounts/${encodeURIComponent(accountId)}/dns_settings/views/${encodeURIComponent(viewId)}`, "GET");
}
