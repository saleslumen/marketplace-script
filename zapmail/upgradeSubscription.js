/**
 * @description Upgrade existing subscription. POST /v2/subscriptions/upgrade.
 * @param {Object} input
...
async function upgradeSubscription(input) {
  ...
  requireField(req, "lookupKey", "string");
  requireField(req, "subscriptionId", "string");
  const body = omit(req, ["serviceProvider", "x-workspace-key", "x-workspace-id"]);
  return zapmailRequest("/v2/subscriptions/upgrade", "POST", { serviceProvider, workspaceKey, body });
}
