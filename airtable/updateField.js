/**
 * @description Update a field name, description, or options. PATCH /v0/meta/bases/{baseId}/tables/{tableId}/fields/{columnId}.
 * @param {Object} input
 * @param {string} input.baseId
 * @param {string} input.tableId
 * @param {string} input.columnId
...
async function updateField(input) {
  ...
  const columnId = requireId(req.columnId, "columnId");
  return airtableRequest(`/meta/bases/${encodeURIComponent(baseId)}/tables/${encodeURIComponent(tableId)}/fields/${encodeURIComponent(columnId)}`, "PATCH", buildUpdateFieldBody(req));
}
