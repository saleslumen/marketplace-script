/**
 * @description Replace one record. PUT /v0/{baseId}/{tableIdOrName}/{recordId}. Fields omitted from fields are cleared.
 * @param {Object} input
 ...
 */
async function replaceRecord(input) {
  const located = requireBaseTable(input);
  const recordId = requireId(located.req.recordId, "recordId");
  return airtableRequest(tablePath(located.baseId, located.tableIdOrName, `/${encodeURIComponent(recordId)}`), "PUT", buildSingleWriteBody(located.req));
}
