/**
 * @description Create up to 200 records with the sObject Collections API.
 ...
 */
async function createSObjects(sobject, records, allOrNone) {
  const type = requireSObject(sobject);
  return {
    results: asArray(await dataRequest("/composite/sobjects", "POST", { allOrNone: optionalFlag(allOrNone) === true, records: withSObjectType(type, records) })),
  };
}
