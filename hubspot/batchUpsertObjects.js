/**
 * @description Create or update a batch of objects by unique property values. POST /crm/objects/{apiVersion}/{objectType}/batch/upsert. inputs is required. idProperty belongs on each input.
 * @param {Object} input
 ...
 */
async function batchUpsertObjects(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  requiredArray(req, "inputs");
  return requestJson(`/crm/objects/${configuredApiVersion()}/${encodeURIComponent(objectType)}/batch/upsert`, "POST", bodyFrom(req, ["objectType"]));
}
