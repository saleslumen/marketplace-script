/**
 * @description Delete pipeline. DELETE /crm/pipelines/{apiVersion}/{objectType}/{pipelineId}. validateDealStageUsagesBeforeDelete and validateReferencesBeforeDelete are optional query parameters.
 * @param {Object} input
 ...
 */
async function deletePipeline(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  const pipelineId = requiredPath(req, "pipelineId");
  return requestJson(`/crm/pipelines/${configuredApiVersion()}/${encodeURIComponent(objectType)}/${encodeURIComponent(pipelineId)}`, "DELETE", undefined, queryFrom(req, ["validateDealStageUsagesBeforeDelete", "validateReferencesBeforeDelete"]));
}
