/**
 * @description Retrieve pipelines. GET /crm/pipelines/{apiVersion}/{objectType}.
 * @param {Object} input
 ...
 */
async function retrievePipelines(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  return requestJson(`/crm/pipelines/${configuredApiVersion()}/${encodeURIComponent(objectType)}`, "GET");
}
