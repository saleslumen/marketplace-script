/**
 * @description Archive a property. DELETE /crm/properties/{apiVersion}/{objectType}/{propertyName}.
 * @param {Object} input
 ...
 */
async function archiveProperty(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  const propertyName = requiredPath(req, "propertyName");
  return requestJson(`/crm/properties/${configuredApiVersion()}/${encodeURIComponent(objectType)}/${encodeURIComponent(propertyName)}`, "DELETE");
}
