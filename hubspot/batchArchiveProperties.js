/**
 * @description Archive a batch of properties. POST /crm/properties/{apiVersion}/{objectType}/batch/archive. inputs is required.
 * @param {Object} input
 ...
 */
async function batchArchiveProperties(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  requiredArray(req, "inputs");
  return requestJson(`/crm/properties/${configuredApiVersion()}/${encodeURIComponent(objectType)}/batch/archive`, "POST", bodyFrom(req, ["objectType"]));
}
