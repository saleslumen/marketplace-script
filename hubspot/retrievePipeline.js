/**
 * @description Retrieve pipeline. GET /crm/pipelines/{apiVersion}/{objectType}/{pipelineId}.
 * @param {Object} input
 ...
 */
async function retrievePipeline(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  const pipelineId = requiredPath(req, "pipelineId");
  return requestJson(`/crm/pipelines/${configuredApiVersion()}/${encodeURIComponent(objectType)}/${encodeURIComponent(pipelineId)}`, "GET");
}
