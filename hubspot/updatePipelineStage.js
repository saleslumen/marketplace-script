/**
 * @description Update stage. PUT /crm/pipelines/{apiVersion}/{objectType}/{pipelineId}/stages/{stageId}. displayOrder, label, and metadata are required. Other input keys are the stage body.
 * @param {Object} input
 ...
 */
async function updatePipelineStage(input) {
  ...
  return requestJson(`/crm/pipelines/${configuredApiVersion()}/${encodeURIComponent(objectType)}/${encodeURIComponent(pipelineId)}/stages/${encodeURIComponent(stageId)}`, "PUT", bodyFrom(req, ["objectType", "pipelineId", "stageId"]));
}
