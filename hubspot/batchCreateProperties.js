/**
 * @description Create a batch of properties. POST /crm/properties/{apiVersion}/{objectType}/batch/create. inputs is required.
 * @param {Object} input
 ...
 */
async function batchCreateProperties(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  requiredArray(req, "inputs");
  return requestJson(`/crm/properties/${configuredApiVersion()}/${encodeURIComponent(objectType)}/batch/create`, "POST", bodyFrom(req, ["objectType"]));
}
