/**
 * @description Archive a batch of objects by ID. POST /crm/objects/{apiVersion}/{objectType}/batch/archive. inputs is required.
 * @param {Object} input
 ...
 */
async function batchArchiveObjects(input) {
  const req = inputObject(input);
  const objectType = requiredPath(req, "objectType");
  requiredArray(req, "inputs");
  return requestJson(`/crm/objects/${configuredApiVersion()}/${encodeURIComponent(objectType)}/batch/archive`, "POST", bodyFrom(req, ["objectType"]));
}
