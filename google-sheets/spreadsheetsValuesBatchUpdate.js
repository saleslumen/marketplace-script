/**
 * @description Sets values in one or more ranges of a spreadsheet. POST /v4/spreadsheets/{spreadsheetId}/values:batchUpdate
 * @param {Object} input
 * @param {string} input.spreadsheetId
 * @param {string} input.valueInputOption INPUT_VALUE_OPTION_UNSPECIFIED, RAW, or USER_ENTERED
 * @param {Object[]} input.data ValueRange objects with range and values
 ...
 * @returns {Object} BatchUpdateValuesResponse
 ...
 */
async function spreadsheetsValuesBatchUpdate(input) {
  ...
  const body = bodyFrom([
    ...
    ["data", requireValueRanges(source.data, "data")],
  ...
  return requestJson(`${spreadsheetPath(spreadsheetId)}/values:batchUpdate`, "POST", body);
}
